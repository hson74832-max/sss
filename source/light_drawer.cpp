...
static std::vector<uint8_t> s_light_buffer;
...
void LightDrawer::draw(int map_x, int map_y, int end_x, int end_y, int scroll_x, int scroll_y, bool fog) {
	...
	// Use static buffer and resize only when necessary
	size_t required_size = static_cast<size_t>(w * h * PixelFormatRGBA);
	if (s_light_buffer.size() < required_size) {
		s_light_buffer.resize(required_size);
	}
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, s_light_buffer.data());
